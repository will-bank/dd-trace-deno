...
const finishExecuteCh = dc.channel('apm:graphql:execute:finish');
const executeErrorCh = dc.channel('apm:graphql:execute:error');
...
// parse channels
const parseStartCh = dc.channel('apm:graphql:parser:start');
const parseFinishCh = dc.channel('apm:graphql:parser:finish');
const parseErrorCh = dc.channel('apm:graphql:parser:error');

// validate channels
const validateStartCh = dc.channel('apm:graphql:validate:start');
const validateFinishCh = dc.channel('apm:graphql:validate:finish');
const validateErrorCh = dc.channel('apm:graphql:validate:error');

function getOperation(document: { definitions: any[] }, operationName: undefined) {
  ...
  if (operationName) {
    return definitions
      .filter((def: { operation: string }) => types.indexOf(def.operation) !== -1)

      .find((def: { name: { value: any } }) => operationName === (def.name && def.name.value));
  } else {

    return definitions.find((def: { operation: string }) => types.indexOf(def.operation) !== -1);
  }
}

function normalizeArgs(args: string | IArguments | any[], defaultFieldResolver) {
...
function normalizePositional(args: string | any[], defaultFieldResolver) {
...
function wrapParse(parse: { apply: (arg0: any, arg1: IArguments) => any }) {
  return function (source: { body: any }) {
...
function wrapValidate(validate: { apply: (arg0: any, arg1: IArguments) => any }) {
...
function wrapExecute(execute: { defaultFieldResolver: any }) {
  return function (exe: { apply: (arg0: any, arg1: IArguments) => any }) {
    ...
    return function () {
      ...
      return asyncResource.runInAsyncScope(() => {
        ...
        return callInAsyncScope(exe, asyncResource, this, arguments, (err, res: { errors: any[] }) => {
...
function wrapResolve(resolve: { apply: (arg0: any, arg1: IArguments) => any }) {
  if (typeof resolve !== 'function' || patchedResolvers.has(resolve)) return resolve;


  function resolveAsync(source, args, contextValue, info: { path: any }) {
...
function callInAsyncScope(
  fn: { apply: (arg0: any, arg1: any) => any },
  aR: { runInAsyncScope: (arg0: () => any) => any; bind: (arg0: { (res: any): any; (err: any): any }) => any },
  thisArg,
  args: IArguments,
  cb: { (err: any, res: any): void; (err: any): void; (arg0: any, arg1: undefined): void },
) {
  cb = cb || (() => {});
...
function assertField(context: { fields: any }, info: { path: any }, args) {
...
function getParentField(context: { asyncResource: any }, path: string | any[]) {
...
function getField(context: { fields: { [x: string]: any } }, path: any[]) {
  return context.fields[path.join('.')];
}

function wrapFields(type: { _fields: object }) {
...
function wrapFieldResolve(field: { resolve: any }) {
  if (!field || !field.resolve) return;
  field.resolve = wrapResolve(field.resolve);
}

function wrapFieldType(field: { type: any }) {
...
addHook(
  { name: 'graphql', file: 'execution/execute.js', versions: ['>=0.10'] },
  (execute: { defaultFieldResolver: any }) => {
