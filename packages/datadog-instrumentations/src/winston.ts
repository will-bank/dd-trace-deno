...
addHook({ name: 'winston', file: 'lib/winston/logger.js', versions: ['>=3'] }, (Logger: { prototype: any }) => {
  const logCh = dc.channel('apm:winston:log');
  shimmer.wrap(Logger.prototype, 'write', (write: { apply: (arg0: any, arg1: IArguments) => any }) => {
...
addHook(
  { name: 'winston', file: 'lib/winston/logger.js', versions: ['1', '2'] },
  (logger: { Logger: { prototype: { configure: any } } }) => {
    const logCh = dc.channel('apm:winston:log');
    if (logger.Logger.prototype.configure) {
      shimmer.wrap(
        logger.Logger.prototype,
        'configure',
        (configure: { apply: (arg0: any, arg1: IArguments) => any }) => wrapMethod(configure, logCh),
      );
    }
    shimmer.wrap(
      logger.Logger.prototype,
...
function wrapMethod(
  method: { apply: (arg0: any, arg1: IArguments) => any },
  logCh: { hasSubscribers: any; publish: (arg0: { message: any }) => void },
) {
