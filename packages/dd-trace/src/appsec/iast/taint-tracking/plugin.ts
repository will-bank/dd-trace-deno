import { SourceIastPlugin } from '../iast-plugin.ts';
import { getIastContext } from '../iast-context.ts';
import { storage } from '../../../../../datadog-core/index.ts';
import { newTaintedString, taintObject } from './operations.ts';
const {
  HTTP_REQUEST_BODY,
...
class TaintTrackingPlugin extends SourceIastPlugin {
  ...
  _taintTrackingHandler(
    ...
    target: { [x: string]: any },
    property: string,
    iastContext = getIastContext(storage.getStore()),
  ) {
    if (!property) {
  ...
  taintUrl(req: { url: any }, iastContext) {
    this.execSource({
      handler: function () {
  ...
  taintRequest(req: { headers: any }, iastContext) {
...
export default new TaintTrackingPlugin();
