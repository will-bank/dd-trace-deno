...
import { contains, intersects, remove } from './range-utils.ts';

import CommandSensitiveAnalyzer from './sensitive-analyzers/command-sensitive-analyzer.ts';
import LdapSensitiveAnalyzer from './sensitive-analyzers/ldap-sensitive-analyzer.ts';
import SqlSensitiveAnalyzer from './sensitive-analyzers/sql-sensitive-analyzer.ts';
import UrlSensitiveAnalyzer from './sensitive-analyzers/url-sensitive-analyzer.ts';
...
class SensitiveHandler {
  ...
  private _valuePattern: RegExp;
  private _sensitiveAnalyzers: any;
  constructor() {
  ...
  redactSource(
    sources: { [x: string]: { redacted: boolean } },
    redactedSources: any[],
    redactedSourcesContext: any[][],
  ...
  writeRedactedValuePart(
    ...
    if (sourceIndex != null) {
      ...
      if (isSensibleSource) {
        ...
        const dedupedSourceRedactionContexts: any[] = [];

        sourceRedactionContext.forEach((_sourceRedactionContext: { start: any; end: any }) => {
...
export default new SensitiveHandler();
