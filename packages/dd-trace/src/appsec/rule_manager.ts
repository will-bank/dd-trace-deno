...
function updateWafFromRC({ toUnapply, toApply, toModify }) {
  ...
  if (newActions.modified) {
    blocking.updateBlockingConfiguration(
      concatArrays(newActions).find((action: { id: string }) => action.id === 'block'),
    );
...
function concatArrays(files: SpyMap) {
...
function mergeRulesData(files: SpyMap) {
...
function rulesReducer(
  existingEntries: { find: (arg0: (entry: any) => boolean) => any; push: (arg0: any) => void },
  rulesDataEntry: { value: any; expiration: number },
) {
  const existingEntry = existingEntries.find((entry: { value: any }) => entry.value === rulesDataEntry.value);
...
export { applyRules, clearAllRules, updateWafFromRC };
