...
class AgentEncoder {
  ...
  private _traceBytes: any;
  private _stringBytes: any;
  private _writer: any;
  private _debugEncoding: boolean;
  private _traceCount: number;
  ...
  _encodeId(bytes: { length: number; reserve: (arg0: number) => void; buffer: { [x: string]: any } }, id: any[]) {
    const offset = bytes.length;
  ...
  _encodeValue(bytes: { length: number; reserve: (arg0: number) => void; buffer: number[] }, value: string) {
    switch (typeof value) {
      case 'string':
  ...
  _encodeFloat(bytes: { length: number; reserve: (arg0: number) => void; buffer: number[] }, value: number) {
  ...
  _writeArrayPrefix(buffer: number[], offset: number, count: number) {
    buffer[offset++] = 0xdd;
  ...
  _writeTraces(buffer: number[], offset = 0) {
