...
function formatSpan(span: { context: () => any; _startTime: number; _duration: number }) {
...
function setSingleSpanIngestionTags(
  span: { metrics: any } | { service: string | any[]; name: string | any[]; resource: any; type: string | any[] },
  options: { sampleRate: any; maxPerSecond: any },
) {
  if (!options) return;
...
function extractTags(
  trace: { metrics: any; meta: any; error: number } | {
    service: string | any[];
    name: string | any[];
    resource: any;
    type: string | any[];
  ...
  addTag(trace.meta, trace.metrics, 'language', 'typescript');
  addTag(trace.meta, trace.metrics, PROCESS_ID, Deno.pid);
...
function extractRootTags(
  trace: { metrics: any } | { service: string | any[]; name: string | any[]; resource: any; type: string | any[] },
  span:
    | { context: () => any }
    | ({
      resource?: string | any[];
...
function extractError(
  trace: { error: number; meta: any; metrics: any } | {
    service: string | any[];
    name: string | any[];
    resource: any;
    type: string | any[];
  },
  error: { message: any; code: any; name: any; stack: any },
) {
  if (!error) return;
...
function addTag(
  meta: { [x: string]: string } | { metrics: any; meta: any; error: number } | {
    service: string | any[];
    name: string | any[];
    resource: any;
    type: string | any[];
...
function isNodeBuffer(obj: { constructor: { name: string }; readInt8: any; toString: any }) {
  return obj.constructor && obj.constructor.name === 'Buffer' &&
    typeof obj.readInt8 === 'function' &&
    typeof obj.toString === 'function';
}

function isUrl(obj: { constructor: { name: string }; href: any; toString: any }) {
