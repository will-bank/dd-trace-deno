...
export default class CiPlugin extends Plugin {
  ...
  testSessionSpan: any;
  testModuleSpan: any;
  testEnvironmentMetadata: any;
  codeOwnersEntries: any;
  testConfiguration: {
    repositoryUrl: any;
    sha: any;
    osVersion: any;
  ...
  constructor(...args) {
    ...
    this.addSub(`ci:${this.constructor.id}:itr-configuration`, ({ onDone }) => {
      ...
      this.tracer._exporter.getItrConfiguration(this.testConfiguration, (err: { message: any }, itrConfig) => {
        if (err) {
    ...
    this.addSub(`ci:${this.constructor.id}:test-suite:skippable`, ({ onDone }) => {
      ...
      this.tracer._exporter.getSkippableSuites(this.testConfiguration, (err: { message: any }, skippableSuites) => {
        if (err) {
  ...
  startTestSpan(
    ...
    testSuiteSpan: {
      context: () => {
        (): any;
        new (): any;
        _trace: { (): any; new (): any; startTime: any; ticks: any };
        toSpanId: { (): any; new (): any };
        toTraceId: { (): any; new (): any };
        _tags: { (): any; new (): any; [x: string]: any };
        _parentId: { (): any; new (): any; toString: { (arg0: number): any; new (): any } };
      };
    },
    extraTags = {},
  ) {
