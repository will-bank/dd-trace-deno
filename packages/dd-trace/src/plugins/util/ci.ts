...
function parseEmailAndName(emailAndName: string) {
  if (!emailAndName) {
    return { name: '', email: '' };
  }
  let name = '';
...
function normalizeTag(
  targetTags: { [x: string]: any },
  tagKey: string | number,
  normalize: { (filePath: any): string; (repositoryUrl: any): any; (repositoryUrl: any): any; (arg0: any): any },
) {
...
export function getCIMetadata() {
  ...
  if (env.get('GITHUB_ACTIONS') || env.get('GITHUB_ACTION')) {
    const {
      GITHUB_RUN_ID,
      GITHUB_WORKFLOW,
      GITHUB_RUN_NUMBER,
