...
const web = {
  ...
  setConfig(req: { url: any }, config: { filter: (arg0: any) => any; service: any; measured: any }) {
  ...
  startSpan(tracer, config, req: { headers: any }, res, name) {
    const context = this.patch(req);
  ...
  wrapMiddleware(req, middleware: { _name: any; name: any }, name, fn: () => any) {
  ...
  finish(req, error: { name: any; message: any; stack: any }) {
  ...
  finishMiddleware(context: { finished: any; middleware: any[] }) {
  ...
  finishSpan(context: { finished?: any; config?: any; span?: any; req?: any; res?: any }) {
  ...
  finishAll(context: { beforeEnd: any }) {
  ...
  obfuscateQs(config: { queryStringObfuscation: any }, url: string | string[]) {
  ...
  wrapWriteHead(context: { req: any; res: any }) {
    const { req, res } = context;
    const writeHead = res.writeHead;

    return function (statusCode, statusMessage: { [x: string]: any }, headers: { [x: string]: any }) {
  ...
  wrapRes(context, req, res, end: { apply: (arg0: any, arg1: IArguments) => any }) {
    return function () {
  ...
  wrapEnd(context: { tracer: { scope: () => any }; req: any; res: any; span: any }) {
...
function isOriginAllowed(req: { headers: { [x: string]: any } }, headers: { [x: string]: any }) {
...
function splitHeader(str: string) {
  return typeof str === 'string' ? str.split(/\s*,\s*/) : [];
}

function reactivate(req, fn: () => any) {
  const context = contexts.get(req);
...
function addRequestTags(
  context: { req: any; span: any; config: any } | { finished?: any; config?: any; span?: any; req?: any; res?: any },
) {
...
function addResponseTags(
  context: { req: any; res: any; paths: any; span: any } | {
    finished?: any;
    config?: any;
    span?: any;
    req?: any;
...
function addResourceTag(
  context: { req: any; span: any } | { finished?: any; config?: any; span?: any; req?: any; res?: any },
) {
...
function addHeaders(
  context: { req: any; res: any; config: any; span: any } | { req: any; span: any; config: any } | {
    finished?: any;
    config?: any;
    span?: any;
    req?: any;
...
function extractURL(req: { headers: { [x: string]: any }; stream: any; originalUrl: any; url: any }) {
...
function getProtocol(req: { socket: { encrypted: any }; connection: { encrypted: any } }) {
...
function getHeadersToRecord(config: { headers: any[]; hasOwnProperty: (arg0: string) => any }) {
...
function getStatusValidator(config: { validateStatus: any; hasOwnProperty: (arg0: string) => any }) {
  if (typeof config.validateStatus === 'function') {
    return config.validateStatus;
  } else if (config.hasOwnProperty('validateStatus')) {
    log.error('Expected `validateStatus` to be a function.');
  }
  return (code: number) => code < 500;
}

function getHooks(config: { hooks: { request: any } }) {
  const noop = () => {};
...
function getMiddlewareSetting(config: { middleware: any; hasOwnProperty: (arg0: string) => any }) {
...
function getQsObfuscator(config: { queryStringObfuscation: any; hasOwnProperty: (arg0: string) => any }) {
