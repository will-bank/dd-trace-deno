...
import { snapshotKinds } from './constants.ts';

function maybeSourceMap(sourceMap, SourceMapper: { create: (arg0: any[], arg1: any) => any }, debug) {
  if (!sourceMap) return;
...
class Profiler extends EventEmitter {
  ...
  start(options: { logger: { error: (arg0: any) => void } }) {
    this._start(options).catch((err) => {
      if (options.logger) options.logger.error(err);
    });
    return this;
  }


  async _start(options: { logger: { error: (arg0: any) => void } }) {
    ...
    let mapper;
    try {
      const { setLogger, SourceMapper } = await import('@datadog/pprof');
  ...
  async _collect(snapshotKind) {
    ...
    const start = this._lastStart;
    const end = new Date();
    const profiles: ({ profiler: any; profile: any })[] = [];
  ...
  _submit(profiles: object, start, end: Date, snapshotKind) {
...
class ServerlessProfiler extends Profiler {
  private _profiledIntervals: number;
  private _interval: number;
  private _flushAfterIntervals: any;
  constructor() {
    super();
...
export { Profiler, ServerlessProfiler };
